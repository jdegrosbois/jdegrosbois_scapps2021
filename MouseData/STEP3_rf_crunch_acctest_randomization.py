...
for axis in ['x','y','r']:
    ...
    for repeats in range(0,100):
        ...
        no_jump = all_data[all_data["Jump"]==0]
        yes_jump_all = all_data[all_data["Jump"]==1]
        temp = pd.array([True]* 162 + [False]*21)
        np.random.shuffle(temp)
        yes_jump = yes_jump_all[temp]
    ...
    if axis == 'x':
       ...
       rdf = pd.DataFrame(raccuracy)
       rdf.to_csv('r_accuracy_bootstrap.csv',index=False,header=False)
